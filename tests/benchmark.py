...
def tiny_profiler(action):
    ...
    handler = Handler(stop_tracing_module_patterns=[], skip_action_modules=[], action=action)
    tracer = Tracer()
...
#    action = foo # lambda x: None
    ...
    profile_overhead_per_iteration = (profiled_load_duration - load_duration) / iterations
    event_count = 0
    while not trace_event_queue.empty():
        trace_event_queue.get()
        event_count+=1
    print(generate_load, f"overhead: {profile_overhead_per_iteration/1000000} ms, {event_count}")
